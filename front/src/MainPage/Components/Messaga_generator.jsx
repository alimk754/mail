    ...
    try {
      await Promise.all(
        Array.from(selectedMessages).map(async (messageId) => {
          if (title === "Trash") {
            await axios.delete(`http://localhost:8080/api/delete/${messageId}`);
          }else if(title==="Drafts"){
            await axios.delete(`http://localhost:8080/api/deleteDraft/${messageId}`); 
                  
            
          }else if(title!=="Inbox" &&title!=="Sent Mails"&&title!=="Draft") {  
            await axios.delete(`http://localhost:8080/api/folder/${user.email}/${title}/${messageId}`);

          }else{
             if (messages.find(m => m.id === messageId)?.from === user.email) {
               await axios.delete(`http://localhost:8080/api/${messageId}/false`);
            } else {
              await axios.delete(`http://localhost:8080/api/mess/${messageId}`);
            }
